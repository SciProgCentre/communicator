...
private class Dump(val thisType: Type, val apiType: Type, val apiMetadata: ApiMetadata) {
    ...
    fun dump(): Class<*> {
        ...
        apiMetadata.mapping.forEach { (function, declaration) ->
            ...
            classWriter.visitMethod(
                ...
                val parametersTypes = HashMap<Int, Type>()
                ...
                when (val s = parametersVars.size - 1) {
                    ...
                    1 -> {
                        ...
                        boxingIfNeeded(type, function.valueParameters.first().type.classifier as KClass<*>)
                    }
                        ...
                        parametersVars.dropLast(1).forEachIndexed { idx, parameterVar ->
                            ...
                            boxingIfNeeded(type, function.valueParameters[idx].type.classifier as KClass<*>)
                            astore(OBJECT_TYPE)
                        }
    ...
    private fun InstructionAdapter.boxingIfNeeded(type: Type, kotlinClass: KClass<*>) {
        if (kotlinClass.isValue) {
            val inlineClassBox = kotlinClass.asm
            invokestatic(inlineClassBox, "box-impl", inlineClassBox(type), false)
            return
        }

        fun doBoxing(primitiveType: Type, boxType: Type) {
            invokestatic(boxType, "valueOf", boxType(primitiveType), false)
        }
