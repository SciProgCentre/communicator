...
internal fun CommunicatorContext.resolveType(type: KType): Codec<*> {
    ...
    require(classifier is KClass<*>) { "Types referenced in API class can't use type parameters." }
    require(!type.isMarkedNullable) { "Nullable types use in API class isn't allowed." }
    codecsRegistry[type.classifier]?.let { return it }
    if (classifier.serializerOrNull() != null) return serializableFormat(classifier)
...
internal fun <T : Any> CommunicatorContext.scanInterface(
    ...
    require(classOfT.memberProperties.isEmpty()) { "API class can't contain properties" }
    require(classOfT.memberExtensionProperties.isEmpty()) { "API class can't contain extension properties." }
    require(classOfT.memberExtensionFunctions.isEmpty()) { "API class can't contain extension functions." }

    val functions = classOfT.memberFunctions.filterNot {
        it.isEquals() || it.isHashCode() || it.isToString()
    }

    require(functions.all(KFunction<*>::isAbstract)) { "All functions in API class must be abstract." }

    require(
        functions.map(KFunction<*>::name).let {
            val mutSet = hashSetOf<String>()
            it.all(mutSet::add)
        }
    ...
    functions.forEach { function ->
        ...
        require(function.valueParameters.none(KParameter::isOptional)) {
...
//        function.valueParameters.withIndex().filter { (it.type.classifier as KClass<*>).isValue }.forEach { inlineClassParametersMap[function.name to] }
